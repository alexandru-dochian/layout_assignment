...
function solve(currentNode: Node, x: number = 0, y: number = 0): boolean {
...
function printTree(currentNode: Node, depth: number = 0): void {
    let tabs = "";
...
function addToAssignedPositionsArray(position: Position): void {
    if (!isPositionAlreadyAssigned(position)) {
        assignedPositions.push(position);
    }
}

function removeFromAssignedPositionsArray(positionToRemove: Position): void {
    assignedPositions = assignedPositions.filter(
        (position: Position) => !arePositionsEqual(position, positionToRemove)
    );
}

function isPositionAlreadyAssigned(searchedPosition: Position): boolean {
    for (let index = 0; index < assignedPositions.length; index++) {
        const currentPosition: Position = assignedPositions[index];
...
function isPositionCrossingLine(positionToCheck: Position): boolean {
    ...
    for (let index = 0; index < assignedPositions.length; index++) {
        ...
        if (
            positionToCheckColumn == currentPositionColumn &&
...
function arePositionsEqual(firstPosition: Position, secondPosition: Position) {
    return (
